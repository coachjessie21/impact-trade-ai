...
function validateReport(blocks) {
  // Block 1-5 需要實質內容（≥80 字）；Block 6 只是 2 句 intro（≥30 字）
  for (var i = 1; i <= 5; i++) {
...
function buildPrompt(data) {
  ...
  var userPrompt =
    ...
    '【BLOCK6 — 下一步：覺心營顧問服務】\n' +
    '只寫 2 句話，不多不少：\n' +
    '第一句：用這份報告的具體數字（關稅增幅 ' + data.diff_pct + '、情境A毛利 ' + data.scen_a_margin + '）說明企業下一步面臨的選擇——是報價策略調整、是買家談判、還是供應鏈評估，直接點出核心選項。\n' +
...
function buildEmailHtml(data, claudeText) {
  ...
  var html =
    ...
    '<div style="font-size:15px;color:#1a1a2e;line-height:1.85;margin-bottom:16px;">' + renderMarkdown(blocks.b6 || '') + '</div>' +
    // 品牌三元素
    '<div style="display:flex;gap:8px;margin-bottom:16px;">' +
    '<span style="font-size:12px;font-weight:700;background:rgba(255,114,0,0.12);color:#FF7200;border-radius:6px;padding:4px 10px;">AI</span>' +
...
function buildCbamPrompt(data) {
  ...
  var userPrompt =
    ...
    '【BLOCK6 — 覺心營 ESG 顧問服務】\n' +
    '只寫 2 句話，不多不少：\n' +
    '第一句：用這份報告的具體數字（年度 CBAM €' + data.cbamEur + '、' + data.productCategory + '）說明企業接下來要處理的核心問題——是取得碳排強度認證、是跟歐盟買家談分攤、還是啟動減排投資評估，直接點出最緊迫的選項。\n' +
